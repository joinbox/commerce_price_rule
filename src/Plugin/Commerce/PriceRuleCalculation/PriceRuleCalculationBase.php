...
abstract class PriceRuleCalculationBase extends PluginBase implements
  ...
  /**
   * Constructs a new PriceRuleCalculationBase object.
   *
  ...
  protected function assertEntity(EntityInterface $entity) {
    ...
    $calculation_entity_type_id = $this->getEntityTypeId();
    if ($entity_type_id != $calculation_entity_type_id) {
      throw new \InvalidArgumentException(
        sprintf(
          'The price rule calculation requires a "%s" entity, but a "%s" entity was given.',
          $calculation_entity_type_id,
          $entity_type_id
        )
      );
    }
